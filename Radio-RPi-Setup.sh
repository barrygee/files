...
if [ ! -f ~/SETUP_SWITCH.tmp ] ; then

    echo 'Creating SETUP_SWITCH.tmp'
    cd ~/ &&

    # creates a temporary file which is used as a switch to determine which part of the setup script to run 
    touch SETUP_SWITCH.tmp &&
    ...
    echo 'Removing ~/SETUP_SWITCH.tmp'
    rm ~/SETUP_SWITCH.tmp &&
